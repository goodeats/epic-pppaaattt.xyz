...
export function ArtboardSelector({
	...
	const currentArtboard = artboards.find(
		artboard => artboard.slug === params.slug,
	)
	...
	return (
		<Popover open={open} onOpenChange={setOpen} {...props}>
			<PopoverTrigger asChild>
				...
				>
					{currentArtboard?.name ? currentArtboard.name : 'Load an artboard...'}
					<Icon
						name="caret-sort"
				...
				<Command>
					...
					<CommandGroup heading="Examples">
						{artboards.map(artboard => (
							<CommandItem key={artboard.id} onSelect={() => setOpen(false)}>
								<Link to={artboard.slug}>{artboard.name}</Link>
								<Icon
									name="check"
									className={cn(
										'ml-auto h-4 w-4',
										currentArtboard?.id === artboard.id
											? 'opacity-100'
											: 'opacity-0',
									)}
								/>
