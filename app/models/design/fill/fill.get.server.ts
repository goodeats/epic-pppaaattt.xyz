...
const validateQueryWhereArgsPresent = (where: queryWhereArgsType) => {
	...
	if (Object.keys(missingValues).length > 0) {
		...
		throw new Error(
			'Null or undefined values are not allowed in query parameters for design fill.',
		)
	}
}

export const getDesignFill = async ({
	where,
}: {
	where: queryWhereArgsType
}): Promise<IDesignFill | null> => {
	...
	invariant(design, 'Design Fill Not found')
	return deserializeDesign({ design }) as IDesignFill
}
